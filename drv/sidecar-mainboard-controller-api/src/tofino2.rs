...
impl Sequencer {
    ...
    pub fn state(&self) -> Result<Tofino2State, FpgaError> {
        let v = self.read_masked(
            Addr::TOFINO_SEQ_STATE,
            Reg::TOFINO_SEQ_STATE::STATE,
        )?;
        Tofino2State::from_u8(v).ok_or(FpgaError::InvalidValue)
    }

    pub fn error(&self) -> Result<Tofino2Error, FpgaError> {
        let v = self.read_masked(
            Addr::TOFINO_SEQ_ERROR,
            Reg::TOFINO_SEQ_ERROR::ERROR,
        )?;
        Tofino2Error::from_u8(v).ok_or(FpgaError::InvalidValue)
    }
